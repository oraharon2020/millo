...
interface TailoredDesignProps {
  data?: HeroSection | null;
}

export default function TailoredDesign({ data }: TailoredDesignProps) {
  const content = {
    subtitle: data?.subtitle || defaultData.subtitle,
    cta_text: data?.cta_text || defaultData.cta_text,
    cta_link: data?.cta_link || defaultData.cta_link,
    main_image_url: data?.main_image_url || defaultData.main_image_url,
    main_image_position: data?.main_image_position || defaultData.main_image_position,
    secondary_image_url: data?.secondary_image_url || defaultData.secondary_image_url,
    secondary_image_position: data?.secondary_image_position || defaultData.secondary_image_position,
  };

  return (
    <section className="container mx-auto px-4 md:px-6 lg:px-12 py-10 md:py-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-8">
        ...
        <div className="flex flex-col gap-4 md:gap-6 order-2 lg:order-1">
          ...
          <div className="relative h-[180px] md:h-[260px] rounded-[20px] md:rounded-[25px] overflow-hidden order-2">
            {content.secondary_image_url ? (
              <Image 
                key={content.secondary_image_url}
                src={content.secondary_image_url} 
                alt="Secondary" 
                fill 
                className="object-cover" 
                style={{ objectPosition: content.secondary_image_position || 'center center' }}
              />
            ) : (
              <>
        ...
        <div className="relative h-[280px] md:h-[450px] lg:h-[550px] overflow-hidden rounded-[20px] md:rounded-[30px] order-1 lg:order-2">
          {content.main_image_url ? (
            <Image 
              key={content.main_image_url}
              src={content.main_image_url} 
              alt="Kitchen" 
              fill 
              className="object-cover" 
              style={{ objectPosition: content.main_image_position || 'center center' }}
            />
          ) : (
            <>
